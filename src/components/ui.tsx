...
const variantClasses: Record<ButtonVariant, string> = {
  ...
  gold:
    "bg-[#2C5A82] text-[#14171F] shadow-[0_4px_24px_rgba(44, 90, 130,0.3)] hover:shadow-[0_8px_40px_rgba(44, 90, 130,0.4)] hover:-translate-y-0.5 active:scale-[0.97] border border-transparent",
  text:
    ...
    return (
      <div className="w-full">
        ...
        <div className="relative">
          {icon ? (
            <div className="absolute left-4 top-1/2 -translate-y-1/2 text-[#909090] pointer-events-none">
              {icon}
            </div>
          ) : null}
          <input
            ...
            id={id}
            className={cn(
              "w-full rounded-xl border border-[#14171F]/15 bg-white px-4 py-3.5 text-sm text-[#14171F] outline-none transition-all duration-200 placeholder:text-[#909090] focus:border-[#2C5A82] focus:shadow-[0_0_0_3px_rgba(44, 90, 130,0.15)]",
              icon && "pl-11",
        ...
        {error ? (
          ...
          <p className="mt-1.5 text-xs text-[#909090]">{hint}</p>
        ) : null}
      </div>
    );
  }
      ...
      <div className="w-full">
        ...
        <div className="relative">
          <select
            ...
            id={id}
            className={cn(
              "w-full appearance-none rounded-xl border border-[#14171F]/15 bg-white px-4 py-3.5 pr-11 text-sm text-[#14171F] outline-none transition-all duration-200 focus:border-[#2C5A82] focus:shadow-[0_0_0_3px_rgba(44, 90, 130,0.15)]",
              error && "border-[#EF4444] focus:border-[#EF4444] focus:shadow-[0_0_0_3px_rgba(239,68,68,0.15)]",
              props.disabled && "bg-[#FAF9F5] cursor-not-allowed opacity-60",
              className
            )}
          ...
          <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 h-4 w-4 text-[#909090] pointer-events-none" />
        </div>
        {error ? (
          <p className="flex items-center gap-1.5 mt-1.5 text-xs text-[#EF4444]">
            <AlertCircle className="h-3.5 w-3.5 shrink-0" />
...
export function DashboardCard({
  ...
  return (
    ...
    >
      ...
      {description ? <p className="mt-0.5 text-xs text-[#909090]">{description}</p> : null}
    </div>
  );
}
    ...
    <div className={cn("flex flex-col items-center gap-6 rounded-xl border border-dashed border-[#14171F]/10 bg-white/60 px-6 py-16 sm:py-20 text-center", className)}>
      {icon ? (
        ...
        <div className="flex h-16 w-16 items-center justify-center rounded-full bg-[#2C5A82]/10">
          <span className="font-editorial text-3xl text-[#2C5A82]">âˆ…</span>
        </div>
      )}
      <div>
