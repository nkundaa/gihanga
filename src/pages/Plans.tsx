          ...
          <div className="mt-14 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {plans.map((p) => (
              <article key={p.name} data-reveal className={`relative rounded-[2.4rem] p-7 transition hover:-translate-y-1 sm:p-8 ${p.featured ? "bg-[#14171F] text-white shadow-[0_30px_110px_rgba(0,0,0,0.18)]" : "border border-black/[0.08] bg-white"}`}>
                {p.featured ? <span className="absolute right-5 top-5 rounded-full bg-[#2C5A82] px-3 py-1 text-xs font-black uppercase tracking-[0.2em] text-[#111]">Most popular</span> : null}
                <p className="text-2xl">{p.emoji}</p>
                <p className={`mt-3 text-xs font-black uppercase tracking-[0.28em] ${p.featured ? "text-[#2C5A82]" : "text-[#666]"}`}>{p.name}</p>
                ...
                <ul className="mt-6 space-y-2.5 text-sm">
                  {p.features.map((f) => (
                    <li key={f} className="flex items-start gap-2.5">
                      <CheckCircle2 className={`mt-0.5 h-4 w-4 shrink-0 ${p.featured ? "text-[#2C5A82]" : "text-[#111]"}`} />
