      ...
      <div className="mx-auto max-w-4xl px-4 pb-24 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-4">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.36em] text-[#2C5A82]">Order</p>
              <h1 className="mt-2 font-display text-[clamp(1.4rem,4.5vw,4.5rem)] font-black leading-[0.92] tracking-[-0.06em]">#{order.id}</h1>
              <p className="mt-1 text-sm text-[#6D6D6D]">{order.createdAt}</p>
            </div>
        ...
        {isCancelled ? (
          ...
          <div className="mb-8 rounded-[2rem] border border-black/[0.08] bg-white p-6 shadow-[0_12px_40px_rgba(0,0,0,0.04)] sm:p-8">
            <p className="text-xs font-black uppercase tracking-[0.28em] text-[#2C5A82]">Tracking</p>
                ...
                {statusSteps.map((step, i) => {
                  ...
                  return (
                    <div key={step.key} className="flex flex-col items-center">
                      <div className={cn("flex h-10 w-10 items-center justify-center rounded-full transition", isActive ? "bg-[#14171F] text-[#2C5A82]" : "bg-[#FAF9F5] text-[#6D6D6D]")}>
                        <Icon className="h-5 w-5" strokeWidth={1.8} />
                      </div>
                      <p className={cn("mt-2 text-[0.55rem] font-bold uppercase tracking-[0.15em] sm:text-xs", isActive ? "text-[#14171F]" : "text-[#6D6D6D]")}>{step.label}</p>
                    </div>
                  );
                })}
              </div>
              <div className="relative mt-2 h-1 rounded-full bg-[#FAF9F5]">
                <div className="absolute left-0 top-0 h-full rounded-full bg-[#14171F] transition-all" style={{ width: `${Math.max(0, currentStepIndex) / (statusSteps.length - 1) * 100}%` }} />
              </div>
            </div>
          </div>
        )}
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <section className="rounded-[2rem] border border-black/[0.08] bg-white p-6 shadow-[0_12px_40px_rgba(0,0,0,0.04)] sm:p-8">
            <p className="text-xs font-black uppercase tracking-[0.28em] text-[#2C5A82]">Delivery</p>
            <div className="mt-4 space-y-3">
              <div className="flex items-center gap-3">
                <MapPin className="h-4 w-4 shrink-0 text-[#2C5A82]" />
                <div>
                  <p className="text-sm font-bold">{order.customer}</p>
                  <p className="text-xs text-[#6D6D6D]">{order.address}</p>
                  {order.deliveryNotes ? <p className="text-xs text-[#6D6D6D]">Note: {order.deliveryNotes}</p> : null}
                </div>
              </div>
              <div className="flex items-center gap-3">
                <Phone className="h-4 w-4 shrink-0 text-[#2C5A82]" />
          ...
          <section className="rounded-[2rem] border border-black/[0.08] bg-white p-6 shadow-[0_12px_40px_rgba(0,0,0,0.04)] sm:p-8">
            ...
            <div className="mt-4 space-y-3">
              <div className="flex items-center gap-3">
                <CreditCard className="h-4 w-4 shrink-0 text-[#2C5A82]" />
                <p className="text-sm font-bold capitalize">{order.payment?.replace("_", " ") ?? "N/A"}</p>
              </div>
              <div className="flex items-center gap-3">
                <Truck className="h-4 w-4 shrink-0 text-[#2C5A82]" />
        ...
        <section className="mt-6 rounded-[2rem] border border-black/[0.08] bg-white p-6 shadow-[0_12px_40px_rgba(0,0,0,0.04)] sm:p-8">
          <p className="text-xs font-black uppercase tracking-[0.28em] text-[#2C5A82]">Items</p>
          <div className="mt-4 space-y-3">
            {order.lines?.map((item, i) => (
              <div key={i} className="flex items-center gap-4 rounded-2xl bg-[#FAF9F5] p-4">
                ...
                <div className="flex flex-1 items-center justify-between gap-3 min-w-0">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-[#6D6D6D]">Qty {item.quantity}{item.size ? ` · ${item.size}` : ""}{item.color ? ` · ${item.color}` : ""}</p>
                  </div>
