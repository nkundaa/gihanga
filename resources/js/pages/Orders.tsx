    ...
    <div className="overflow-x-hidden bg-[#F8F9FA] pt-24 lg:pt-28">
      ...
      <div className="mx-auto max-w-5xl px-4 pb-24 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <p className="mt-2 text-[#666666]">Track and manage all your purchases.</p>
        </div>

        <div className="mb-6 flex flex-wrap gap-2">
          {statuses.map((s) => (
            <button key={s} type="button" onClick={() => { const next = new URLSearchParams(); if (s !== "all") next.set("status", s); setParams(next); }}
              className={cn("min-h-11 rounded-full border px-3 py-2 text-[0.6rem] font-bold uppercase tracking-[0.18em] transition", statusFilter === s ? "border-[#111111] bg-[#111111] text-white" : "border-black/10 bg-white text-[#666666] hover:border-black/30")}>
              {s}
            </button>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {filtered.map((order) => (
              <Link key={order.id} to={`/orders/${order.id}`} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-black/[0.06] bg-white p-5 transition hover:bg-[#F8F9FA]">
                <div className="min-w-0">
                  <p className="font-display text-base font-black tracking-[-0.02em] truncate">Order #{order.id}</p>
                  <p className="text-sm text-[#666666] truncate">{order.createdAt} · {order.storeName} · {order.lines?.length ?? 0} item{(order.lines?.length ?? 0) !== 1 ? "s" : ""}</p>
                </div>
