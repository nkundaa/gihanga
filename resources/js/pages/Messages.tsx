    ...
    <div className="overflow-x-hidden bg-[#FAF9F5] pt-24 lg:pt-28">
      ...
      <div className="mx-auto max-w-4xl px-4 pb-24 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <p className="mt-2 text-[#6D6D6D]">Your conversations with stores.</p>
        </div>

        {loading ? (
          <div className="flex justify-center py-20"><div className="h-8 w-8 animate-spin rounded-full border-2 border-[#2C5A82] border-t-transparent" /></div>
          ...
          <div className="space-y-2">
            {conversations.map((c) => (
              <Link key={c.id} to={`/messages/${c.id}`}
                className="flex items-center gap-4 rounded-2xl border border-black/[0.06] bg-white p-4 transition hover:bg-[#FAF9F5]">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[#2C5A82]/20">
                  <Store className="h-5 w-5 text-[#2C5A82]" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-bold truncate">{c.store?.name ?? "Store"}</p>
                    {c.is_closed ? <span className="rounded-full bg-[#FAF9F5] px-2 py-0.5 text-[0.5rem] font-bold uppercase tracking-[0.15em] text-[#6D6D6D]">Closed</span> : null}
                  </div>
                  <p className="text-xs text-[#6D6D6D] mt-0.5 truncate">{c.last_message?.content ?? c.subject ?? "No messages yet"}</p>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-[0.6rem] text-[#6D6D6D]">{c.last_message ? new Date(c.created_at).toLocaleDateString() : ""}</p>
                </div>
              </Link>
            ))}
          </div>
