...
export default function Wishlist() {
  ...
  return (
    <div className="overflow-x-hidden bg-[#FAF9F5]">
      ...
      <section className="relative flex min-h-[25svh] items-center overflow-hidden bg-[#14171F] py-10 text-white sm:py-12 sm:min-h-[30svh] pt-28">
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
          <p className="text-[0.6rem] font-black uppercase tracking-[0.3em] text-[#2C5A82] sm:text-xs sm:tracking-[0.42em]">Your wishlist</p>
          <h1 className="mt-3 max-w-5xl font-display text-[clamp(1.3rem,5vw,5.2rem)] font-black uppercase leading-[0.94] tracking-[-0.08em] sm:mt-4">
            Saved <span className="font-editorial normal-case text-[#2C5A82]">pieces</span><br />
          ...
          {count > 0 ? (
            <button type="button" onClick={clearWishlist} className="mt-3 min-h-11 text-[0.6rem] font-bold uppercase tracking-[0.18em] text-[#2C5A82] underline-grow sm:mt-4 sm:text-xs sm:tracking-[0.2em]">
              Clear wishlist
            </button>
          ) : null}
        </div>
      ...
      {count > 0 ? (
        <section className="overflow-x-hidden bg-white px-4 py-16 sm:px-6 lg:px-8 sm:py-20">
          <div className="mx-auto max-w-7xl text-center">
            <Heart className="mx-auto h-8 w-8 text-[#2C5A82]" />
            <h2 className="mt-4 font-display text-[clamp(1.25rem,4vw,2rem)] font-black tracking-[-0.05em]">Ready to checkout?</h2>
            <p className="mt-3 text-[#6D6D6D]">Your saved pieces are waiting. Add them to your bag and complete your order.</p>
            <div className="mt-8 flex justify-center gap-3">
              <MagneticButton to="/shop" variant="primary" className="min-h-12 px-6 py-3 text-sm">Continue shopping</MagneticButton>
            </div>
          </div>
        </section>
      ) : null}
