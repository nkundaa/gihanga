  ...
  if (!store) {
    return (
      <div className="overflow-x-hidden flex min-h-[80svh] flex-col items-center justify-center gap-6 px-4 pt-36 text-center">
        ...
        <MagneticButton to="/stores" variant="berry" className="min-h-12 px-6 py-3 text-sm">Back to stores</MagneticButton>
      </div>
    );
  }
  ...
  return (
    <div className="overflow-x-hidden bg-[#F8F9FA]">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="absolute left-0 right-0 top-28 z-10 px-4 sm:px-6 lg:px-8">
          ...
          <aside data-reveal className="space-y-5 lg:sticky lg:top-28 lg:self-start">
            ...
            <div className="rounded-[2rem] border border-black/[0.08] bg-white p-6 shadow-[0_20px_70px_rgba(0,0,0,0.06)]">
              <ul className="space-y-4 text-sm">
                <li className="flex gap-3"><MapPin className="h-5 w-5 shrink-0 text-[#BFD7F1]" /> <span><strong>{store.location}</strong></span></li>
                <li className="flex gap-3"><Clock className="h-5 w-5 shrink-0 text-[#BFD7F1]" /> <span>{store.hours}</span></li>
                <li className="flex gap-3"><Star className="h-5 w-5 shrink-0 fill-[#BFD7F1] text-[#BFD7F1]" /> <span>{store.rating.toFixed(1)} average rating</span></li>
              </ul>
              <MagneticButton to="/contact" variant="dark" className="mt-6 min-h-12 w-full justify-center px-6 py-3 text-sm">Contact boutique</MagneticButton>
            </div>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <h2 className="mt-3 font-display text-[clamp(1.5rem,4vw,2.5rem)] font-black tracking-[-0.06em]">Other <span className="font-editorial text-[#BFD7F1]">boutiques</span></h2>
            </div>
