    ...
    return (
      <div className="overflow-x-hidden flex min-h-[80svh] flex-col items-center justify-center gap-6 px-4 pt-36 text-center">
        ...
        <div className="mt-4 w-full rounded-2xl border border-black/10 bg-white p-6 text-left shadow-[0_20px_60px_rgba(0,0,0,0.06)]">
          ...
          <p className="text-sm text-[#6D6D6D]">{count} item{count === 1 ? "" : "s"} · {payment === "mobile_money" ? "Mobile Money" : "Card"} payment</p>
        </div>
        ...
        <a href="https://maps.google.com/maps?q=Kigali+Rwanda" target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex min-h-11 items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-[#2C5A82] underline underline-offset-4 transition hover:text-[#14171F]">
                ...
                <div className="mt-4 grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <div className="flex flex-col gap-2 sm:flex-row">
                      ...
                      <button type="button" onClick={detectLocation} disabled={detecting} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl border border-black/10 bg-[#FAF9F5] px-4 py-3 text-sm font-bold text-[#14171F] transition hover:bg-[#2C5A82]/20 hover:border-[#2C5A82] disabled:opacity-50">
                        <MapPin className={cn("h-4 w-4 text-[#2C5A82]", detecting && "animate-bounce")} /> {detecting ? "Detecting…" : "Detect"}
                      </button>
                    </div>
                    {locError && <p className="mt-1.5 text-xs text-red-500">{locError}</p>}
                    {location && (
                      <a href={location.address} target="_blank" rel="noopener noreferrer" className="mt-1.5 inline-flex min-h-11 items-center gap-1.5 text-xs text-[#2C5A82] underline underline-offset-2 transition hover:text-[#14171F]">
                    ...
                    <div key={line.key} className="flex gap-4 rounded-2xl bg-[#FAF9F5] p-4">
                      ...
                      <div className="flex flex-1 items-center justify-between gap-3 min-w-0">
                        <div className="min-w-0">
                          ...
                          <p className="mt-0.5 text-xs text-[#6D6D6D]">Qty {line.quantity}{line.size ? ` · ${line.size}` : ""}{line.color ? ` · ${line.color}` : ""}</p>
                        </div>
