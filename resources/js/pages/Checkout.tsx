  ...
  if (count === 0 && !submitted) {
    return (
      <div className="overflow-x-hidden flex min-h-[80svh] flex-col items-center justify-center gap-6 px-4 pt-36 text-center">
        ...
        <MagneticButton to="/shop" variant="berry" className="min-h-12 px-6 py-3 text-sm">Browse shop</MagneticButton>
      </div>
    );
  }
    ...
    return (
      <div className="overflow-x-hidden flex min-h-[80svh] flex-col items-center justify-center gap-6 px-4 pt-36 text-center">
        ...
        <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row">
          <MagneticButton to="/shop" variant="dark" className="min-h-12 w-full px-6 py-3 text-sm sm:w-auto">Continue shopping</MagneticButton>
          <MagneticButton to="/" variant="light" className="min-h-12 w-full px-6 py-3 text-sm sm:w-auto">Back to home</MagneticButton>
        </div>
        <a href="https://maps.google.com/maps?q=Kigali+Rwanda" target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex min-h-11 items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-[#BFD7F1] underline underline-offset-4 transition hover:text-[#111]">
  ...
  if (!isAuthenticated) {
    return (
      <div className="overflow-x-hidden flex min-h-[80svh] flex-col items-center justify-center gap-6 px-4 pt-36 text-center">
        ...
        <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row">
          <MagneticButton to="/login" variant="berry" className="min-h-12 w-full px-6 py-3 text-sm sm:w-auto">Sign in</MagneticButton>
          <MagneticButton to="/register" variant="dark" className="min-h-12 w-full px-6 py-3 text-sm sm:w-auto">Create account</MagneticButton>
        </div>
      </div>
    );
  }
          ...
          <div className="space-y-8">
              ...
              <form className="rounded-[2rem] border border-black/[0.08] bg-white p-4 shadow-[0_20px_70px_rgba(0,0,0,0.06)] sm:p-8" onSubmit={(e) => e.preventDefault()}>
                ...
                <div className="mt-4 grid gap-4 sm:grid-cols-2">
                  <div className="sm:col-span-2">
                    ...
                    <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="min-h-12 w-full rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm outline-none transition focus:border-[#BFD7F1]" placeholder="Jean Baptiste Mugabo" />
                  </div>
                  <div>
                    <label className="mb-2 block text-xs font-black uppercase tracking-[0.28em] text-[#666666]">Phone</label>
                    <input type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="min-h-12 w-full rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm outline-none transition focus:border-[#BFD7F1]" placeholder="+250 788 000 000" />
                  </div>
                  <div>
                    <label className="mb-2 block text-xs font-black uppercase tracking-[0.28em] text-[#666666]">Email</label>
                    <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="min-h-12 w-full rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm outline-none transition focus:border-[#BFD7F1]" placeholder="jean@example.com" />
                  </div>
                  <div className="sm:col-span-2">
                    <label className="mb-2 block text-xs font-black uppercase tracking-[0.28em] text-[#666666]">Delivery address</label>
                    <input value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} className="min-h-12 w-full rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm outline-none transition focus:border-[#BFD7F1]" placeholder="Kacyiru, KG 123 St, Kigali" />
                  </div>
                  <div className="sm:col-span-2">
                    <label className="mb-2 block text-xs font-black uppercase tracking-[0.28em] text-[#666666]">Your location</label>
                    <div className="flex flex-col gap-2 sm:flex-row">
                      <input readOnly value={location ? `${location.lat.toFixed(6)}, ${location.lng.toFixed(6)}` : ""} className="min-h-12 flex-1 rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm outline-none transition focus:border-[#BFD7F1] placeholder:text-[#aaa]" placeholder="Auto-detected coordinates" />
                      <button type="button" onClick={detectLocation} disabled={detecting} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl border border-black/10 bg-[#F8F9FA] px-4 py-3 text-sm font-bold text-[#111] transition hover:bg-[#BFD7F1]/20 hover:border-[#BFD7F1] disabled:opacity-50">
                    ...
                    {location && (
                      <a href={location.address} target="_blank" rel="noopener noreferrer" className="mt-1.5 inline-flex min-h-11 items-center gap-1.5 text-xs text-[#BFD7F1] underline underline-offset-2 transition hover:text-[#111]">
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <textarea rows={3} value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} className="min-h-12 w-full resize-none rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm outline-none transition focus:border-[#BFD7F1]" placeholder="Landmark, gate number, instructions for the rider..." />
                  </div>
                </div>
                <MagneticButton variant="berry" className="mt-6 min-h-12 w-full sm:w-auto px-6 py-3 text-sm" onClick={() => setStep("payment")}>
                  Continue to payment
                </MagneticButton>
              </form>
            )}
              ...
              <div className="rounded-[2rem] border border-black/[0.08] bg-white p-4 shadow-[0_20px_70px_rgba(0,0,0,0.06)] sm:p-8">
                ...
                <div className="mt-6 grid gap-3">
                  {paymentOptions.map((opt) => (
                    <button key={opt.value} type="button" onClick={() => setPayment(opt.value)} className={cn("flex items-center gap-4 rounded-2xl border p-5 text-left transition min-h-[4.5rem]", payment === opt.value ? "border-[#BFD7F1] bg-[#BFD7F1]/10" : "border-black/10 bg-white hover:border-black/30")}>
                      <span className={cn("flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl", payment === opt.value ? "bg-[#111111] text-[#BFD7F1]" : "bg-[#F8F9FA] text-[#666666]")}>
                      ...
                      {payment === opt.value ? <CheckCircle2 className="ml-auto shrink-0 h-5 w-5 text-[#BFD7F1]" /> : null}
                    </button>
                  ))}
                </div>
                ...
                <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                  <MagneticButton variant="light" className="min-h-12 w-full sm:w-auto px-6 py-3 text-sm" onClick={() => setStep("details")}>Back</MagneticButton>
                  <MagneticButton variant="berry" className="min-h-12 w-full sm:w-auto px-6 py-3 text-sm" onClick={() => setStep("confirm")}>Review order</MagneticButton>
                </div>
              </div>
            )}
            ...
            {step === "confirm" && (
              <div className="rounded-[2rem] border border-black/[0.08] bg-white p-4 shadow-[0_20px_70px_rgba(0,0,0,0.06)] sm:p-8">
                ...
                <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                  <MagneticButton variant="light" className="min-h-12 w-full sm:w-auto px-6 py-3 text-sm" onClick={() => setStep("payment")}>Back</MagneticButton>
                  <MagneticButton variant="berry" className="min-h-12 w-full sm:w-auto px-6 py-3 text-sm" onClick={placeOrder} disabled={submitting}>
          ...
          <aside className="lg:sticky lg:top-28 lg:self-start">
            <div className="rounded-[2rem] border border-black/[0.08] bg-white p-4 shadow-[0_20px_70px_rgba(0,0,0,0.06)]">
              ...
              <div className="mt-3 flex items-center gap-2 text-xs text-[#666666]">
                <Truck className="h-4 w-4 shrink-0 text-[#BFD7F1]" /> Free Kigali delivery included
              </div>
            </div>
          </aside>
        </div>
